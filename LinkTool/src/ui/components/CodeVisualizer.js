...
if(!hex)return 'rgba(128, 128, 128, 0.5)';
hex=hex.replace('#','');
...
var glassBg='rgba(200, 200, 200, 0.5)';
if(typeof ThemeManager!=='undefined'){
...
if(link.type==='ACCESS')y2=dst.y+dst.h/2;
...
points.push({x:x1+15,y:y1});
points.push({x:x1+15,y:busY});
points.push({x:targetEntryX,y:busY});
points.push({x:targetEntryX,y:y2});
...
var leftGutterX=x2-25;
var stepOutX=x1+20;

points=[];
points.push({x:x1,y:y1});
points.push({x:stepOutX,y:y1});
points.push({x:stepOutX,y:busY});
points.push({x:leftGutterX,y:busY});
points.push({x:leftGutterX,y:y2});
points.push({x:x2,y:y2});
...
var optTypes=OptionsManager.getOption('showDataTypesV2');
