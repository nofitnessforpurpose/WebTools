...
function FileEditor(editorelement, callback, types) {
...
function BlockFileEditor(editorelement, callback, types) {
...
function ProcedureFileEditor(editorelement, callback) {
   BlockFileEditor.call(this, editorelement, callback, [3]);
}
...
ProcedureFileEditor.prototype.initialise = function (item) {
   ...
   if (!this.myelement) {
      ...
      extraelement.innerHTML =
         ...
         "<div class='opl-editor-container'>" +
   ...
   if (hl && sc) {
      ...
      sc.oninput = function () {
         ...
         // Sync scroll in case input changes height/scroll
      ...
      // Default state: Editing enabled, highlighting disabled (hidden)
      // But with the new overlay method, we can just toggle the 'highlighting-active' class
      // on the container or the textarea to control visibility of the highlight layer.
      // For now, let's keep the logic similar to before but adapted for overlay.

      // Actually, we want the textarea to ALWAYS be editable.
      // The "Syntax Highlight" button will now toggle the VISIBILITY of the highlight layer underneath.
...
ProcedureFileEditor.prototype.toggleHighlight = function () {
   ...
   if (hl.style.display === "none") {
      // Enable Highlight
      // Update highlight content first
      hl.innerHTML = SyntaxHighlighter.highlight(sc.value);
...
ProcedureFileEditor.prototype.getProcedureData = function () {
   ...
   var txt = document.getElementById("sourcecode").value;
...
function NotePadFileEditor(editorelement, callback) {
   BlockFileEditor.call(this, editorelement, callback, [7]);
}
