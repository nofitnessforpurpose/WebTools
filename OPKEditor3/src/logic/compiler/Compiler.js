...
var OPLCompiler = (function () {
    ...
    var KEYWORDS_SHARED = [
        ...
        'OFF',
        'RAISE', 'DIRW$', 'DISP', 'INPUT', 'KSTAT', 'VIEW', 'MENU'
    ];

    // LZ-Only Keywords
    var KEYWORDS_LZ = [
        'ACOS', 'ASIN', 'CLOCK', 'COPYW', 'DAYS', 'DAYNAME$','DELETEW', 'DOW', 'FINDW', 'MAX', 'MEAN','MENU', 'MENUN', 'MIN', 'MONTH$', 'STD', 'SUM', 'VAR', 'UDG', 'WEEK'
    ];
    ...
    function tokenize(source, targetSystem) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            while (ptr < line.length) {
                ...
                // Hex Literals ($F8)
                if (char === '$') {
                    // Peek next char to see if it is a Hex Digit
                    ...
                    // Fallthrough to operator/punctuation if not a valid hex number
                }
    ...
    function compile(source, options) {
        ...
        try {
            ...
            while (ptr < tokens.length) {
                ...
                if (t.type === 'KEYWORD') {
                    if (t.value === 'GLOBAL' || t.value === 'LOCAL') {
                        ...
                        if (peek() && peek().value === ':') {
                            ...
                            var declList = (isLocal ? locals : globals);
                            ...
                            while (true) { // Loop for comma separated list: LOCAL a, b, c
                                ...
                                if (varNameTok && varNameTok.type === 'IDENTIFIER') {
                                    ...
                                    if (/^M[0-9]$/.test(rawName)) {
                                     ...
                                     // Check for duplicate variable names in the current procedure
            ...
            while (ptr < tokens.length) {
                ...
                if (t.type === 'KEYWORD') {
                    if (t.value === 'PRINT' || t.value === 'LPRINT') {
                        ...
                        var colType = parseExpression(); // Column
                        if (colType !== 0) {
                        ...
                        var rowType = parseExpression(); // Row
                        if (rowType !== 0) {
                    ...
                    if (!isAssign) {
                        ...
                        // Determine if this is a 0-arg builtin
                        ...
                        if (next1 && next1.value === ':') {
                            if (next2 && next2.value === ':') {
                                ...
                                // Single colon.
                                if (isZeroArgBuiltin) {
                                    // Treat as a separator, don't consume it
                                    isCall = true;
                                } else {
                                    next(); // Consume first colon
                                    isExplicitCall = true; // Mark as Explicit (forces PROC)
                            ...
                            if (peek() && peek().value === '(') {
                                if (targetSystem === 'Standard' && !op) {
                                    // XP/Standard Mode: Implicit calls Ident(...) are NOT allowed for Procedures.
                                    isCall = false;
                                } else {
                                    isCall = true;
                                }
