...
// Mock alert since PackImage uses it
global.alert = function (msg) { console.log("[ALERT]", msg); }

// Load PackImage source (need to read and eval or require if module)
// Since PackImage.js is not a module, we wrap it or just paste relevant parts.
// Actually, let's try to simulate the environment or use `unconstrained` loader?
// Easier to just reproduce the `PackImage` logic here since it's simple.

// COPY OF PackImage.js LOGIC (Simplified for Node)
...
PackItem.prototype = {
    ...
    // Add getFullData from recent file view
...
function readItem(inputData, ix) {
    ...
    console.log(`Scanning Item at ${ix}: Len=${len}, Type=${tp} (0x${tp.toString(16)})`);

    var item = new PackItem(inputData, ix, len + 2);
    ix += len + 2;
    // item.setDescription(); // simplified
    item.type = tp & 0x7f; // simplified

    // Logic from Line 439 in PackImage.js
    if (item.type == 0x00) { //  && !item.deleted (removed check for simplicity/worst-case)
        var blocklen = item.data[2] * 256 + item.data[3];
        console.log(`  -> Type 00 (Data Block), BlockLen=${blocklen}`);
...
for (var ix = items.length - 1; ix > 0; ix--) {
    if (ix >= items.length) continue; // Safety
    var prev = items[ix - 1];
    var curr = items[ix];
    var tp1 = prev.type;
    var tp2 = curr.type;

    console.log(`Merge Check: PrevType=${tp1} (0x${tp1.toString(16)}), CurrType=${tp2} (0x${tp2.toString(16)})`);

    if (tp1 >= 2 && tp1 <= 15 && tp2 == 0) {
        console.log("  -> MERGING");
        items[ix - 1].child = items[ix]
        items.splice(ix, 1);
    }
}
...
items.forEach((item, i) => {
    console.log(`Item ${i}: Type=${item.type}, Len=${item.length}, Child=${!!item.child}, ChildOfChild=${!!(item.child && item.child.child)}`);
    if (item.type === 3) { // Proc
        if (item.child) {
            console.log(`  ChildType: ${item.child.type}, ChildLen: ${item.child.length}`);
            if (item.child.child) {
                console.log(`  GrandChildType: ${item.child.child.type}, GrandChildLen: ${item.child.child.length}`);
            } else {
                console.log("  No Grandchild.");
            }
        } else {
