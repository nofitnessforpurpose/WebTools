...
try {
    ...
    const result = decompiler.getRawAnalysis(buffer, "test");
    ...
    console.log("\n--- Variable Map ---");
    Object.keys(result.varMap).forEach(k => {
        const v = result.varMap[k];
        if (v.isGlobal) {
            console.log(`Mapped Global: ${v.name} at offset ${k}`);
        }
    });
