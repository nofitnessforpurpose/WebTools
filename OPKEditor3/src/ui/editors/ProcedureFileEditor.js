...
ProcedureFileEditor.prototype.getProcedureData = function () {
    ...
    // 1. Correct Termination: Ensure at least a single null terminator at end.
    // Logic: Record should end with 0x00. Internal 0x00 are newlines.
    // Trimming was previously failing because we added TWO extra nulls but only trimmed ONE.
    ...
    // Append exactly one null terminator (Sanity Check)
    srcBytes.push(0);

    // Calculate Source Length (Includes the terminator)
...
ProcedureFileEditor.prototype.sanitizeProcedureHeader = function (headerLine, fallbackName, isLiveTyping) {
    ...
    // Extract parameter list if present (anything inside (...) at the end of the line)
    var paramList = "";
    ...
    // Remove anything after colon if colon exists
    ...
    var maxBaseLen = 8 - suffix.length;
    var truncatedBase = baseName.substring(0, maxBaseLen);
    var sanitizedLine;
    if (isLiveTyping && baseName.length <= maxBaseLen) {
...
ProcedureFileEditor.prototype.applyChanges = function () {
    ...
    // Enforce Name Sanitization (Truncate to 8 chars or revert invalid) BEFORE getting data
    var txt = "";
    ...
    // Now get the data (safely using the sanitized text)
    var newdata = this.getProcedureData();
...
ProcedureFileEditor.prototype.compileAndSaveItem = function (targetItem, source, interactive, suppressUpdate) {
    ...
    var parsedHeader = this.sanitizeProcedureHeader(lines[0], targetItem.name);
    var actualName = parsedHeader.name || targetItem.name;
    ...
    if (typeof createBlockFile === 'function') {
        ...
        var newItem = createBlockFile(newData, actualName, 3, true);
