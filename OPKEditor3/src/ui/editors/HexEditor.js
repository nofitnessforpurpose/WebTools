...
HexEditor.prototype.initialise = function (item, startAddress) {
    if (!this.myelement) {
...
HexEditor.prototype.renderHexView = function () {
    ...
    // Use recursive matching to get all data including children
    var data = this.item.getFullData ? this.item.getFullData() : this.item.data;
    ...
    var bytesPerRow = OptionsManager.getOption('hexBytesPerRow') || 16;

    for (var i = 0; i < data.length; i += bytesPerRow) {
        var currentAddr = this.startAddress + i;
        var offset = ("00000000" + currentAddr.toString(16)).substr(-8).toUpperCase();
        var hex = "";
        var ascii = "";
