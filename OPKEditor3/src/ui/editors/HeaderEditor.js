...
HeaderEditor.prototype.initialise = function (item) {
    ...
    if (!this.myelement) {
        ...
        for (var i = 2; i <= 7; i++) grpTime.appendChild(this.createByteDisplay(i, this.colors.byteTime));
        ...
        sizeField.innerHTML =
            ...
            "<select id='packsize' style='width:100%; padding:5px;'>" +
        ...
        // paramContainer.style.flex = '1'; // REMOVED Flex Grow to fix Checksum float

        var createParamField = (id, legend, html) => {
            var f = document.createElement('fieldset');
            f.id = id;
            f.style.border = '1px solid ' + this.colors.byteTime.border;
            f.style.background = this.colors.byteTime.bg;
            f.style.borderRadius = '4px';
            f.style.padding = '15px';
            f.style.margin = '0';
            // f.style.height = '100%'; // REMOVED fixed height to allow natural flow
            f.style.boxSizing = 'border-box';
            f.innerHTML = "<legend style='color:" + this.colors.byteTime.border + "; font-weight:bold;'>" + legend + "</legend>" + html;
            return f;
        };

        var timeContent =
            "<div style='display:flex; flex-direction:column; gap:5px; height:100%;'>" +
            "<div id='grp-year' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Year</label><select id='year' style='width:60%'></select></div>" +
            "<div id='grp-month' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Month</label><select id='month' style='width:60%'></select></div>" +
            "<div id='grp-day' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Day</label><select id='day' style='width:60%'></select></div>" +
            "<div id='grp-hour' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Hour</label><select id='hour' style='width:60%'></select></div>" +
            "<div id='grp-cnt1' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Count 1</label><select id='counter1' style='width:60%'></select></div>" +
            "<div id='grp-cnt2' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Count 2</label><select id='counter2' style='width:60%'></select></div>" +
            "</div>";

        var bootContent =
            "<div style='display:flex; flex-direction:column; gap:5px; height:100%;'>" +
            "<div id='grp-code' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Code</label><select id='code' style='width:60%'></select></div>" +
            "<div id='grp-bootid' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>ID</label><select id='bootid' style='width:60%'></select></div>" +
            "<div id='grp-ver' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Ver</label><select id='version' style='width:60%'></select></div>" +
            "<div id='grp-prio' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Prio</label><select id='priority' style='width:60%'></select></div>" +
            "<div id='grp-addr1' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Addr 1</label><select id='address1' style='width:60%'></select></div>" +
            "<div id='grp-addr2' style='display:flex; justify-content:space-between; align-items:center; min-height:30px;'><label>Addr 2</label><select id='address2' style='width:60%'></select></div>" +
            "</div>";

        var timeDiv = createParamField('timesection', 'Time Stamp', timeContent);
        var bootDiv = createParamField('bootsection', 'Boot Data', bootContent);
...
function DrawLinks(editor) {
    ...
    // 3. Bytes 2-7 (Group) -> Time/Boot Section (Purple)
    var bit4El = document.getElementById("bit4");
    var isBootable = bit4El ? bit4El.checked : false;
    var targetSection = isBootable ? 'bootsection' : 'timesection';
    connections.push({ from: 'ph-group-time', to: targetSection, color: editor.colors.byteTime.stroke });
