        ...
        packs.forEach(function (pack, pIdx) {
            ...
            pack.items.forEach(function (item) {
                ...
                if (item.child && item.child.child && item.child.child.data) {
                    ...
                    // Fix: Correctly calculate Source Offset based on Record Structure
                    if (st.valid && st.type === 'long') {
                        // Long Record: obLen is the TOTAL Block Length.
                        // Structure: 02 80 [BlockLen] [QCodeLen] [QCode...] [SrcLen] [Src...]
                        // baseStart points to QCodeLen field.
                        if (data.length >= baseStart + 2) {
                            var qCodeLen = (data[baseStart] << 8) | data[baseStart + 1];
                            srcOffset = baseStart + 2 + qCodeLen;
                        }
                    } else {
                        // Raw/Legacy: obLen is QCode Length (from first 2 bytes)
                        // Structure: [QCodeLen] [QCode...] [SrcLen] [Src...]
                        srcOffset = baseStart + obLen;
                    }

                    if (srcOffset > 0 && data.length >= srcOffset + 2) {
                    ...
                    if (node) {
                        ...
                        var hasSource = (lnsrc > 0) || (item.text ? true : false);
...
//             console.warn("Please allow popups to view the System Map.");
    ...
    function initChildEnvironment() {
        ...
        // Helper to create glass effect from theme color
        function hexToRgba(hex, alpha) {
            if (!hex) return 'rgba(128, 128, 128, 0.5)'; // Fallback
            hex = hex.replace('#', '');
        ...
        var glassBg = 'rgba(200, 200, 200, 0.5)'; // Default
        if (typeof ThemeManager !== 'undefined') {
            var defs = ThemeManager.getThemeDefinition(ThemeManager.currentTheme);
            if (defs && defs['--list-hover-bg']) {
                glassBg = hexToRgba(defs['--list-hover-bg'], 0.5);
            }
        }
        ...
        style.textContent = `
                    ...
                    /* Type Color Bands (Top Left - Offset 2px) */
                    .node-header.type-int::before,
                    .node-header.type-str::before,
                    .node-header.type-float::before {
                        content: '';
                        position: absolute;
                        left: 5px;
                        top: 0;
                    ...
                    .node-header.type-int::before { background-color: rgba(0, 170, 0, 0.7); }
                    .node-header.type-str::before { background-color: rgba(170, 0, 170, 0.7); }
                    .node-header.type-float::before { background-color: rgba(255, 136, 0, 0.7); }
        ...
        doc.addEventListener('keydown', function (e) {
            // Zoom
            if (e.key === '+' || e.key === '=') {
                var btn = doc.getElementById('btn-zoom-in');
                if (btn) btn.click();
            }
            if (e.key === '-' || e.key === '_') {
                var btn = doc.getElementById('btn-zoom-out');
                if (btn) btn.click();
            }
            ...
            if (e.altKey) {
                ...
                if (key === 'U') {
                    var btn = doc.getElementById('btn-toggle-links'); // Usage
                    if (btn) btn.click();
                    e.preventDefault();
                }
                if (key === 'G') {
            ...
            try {
                ...
                data.links.forEach(function (link) {
                    ...
                    if (src && dst && !drawnLinks[linkId]) {
                        ...
                        if (link.type === 'CALL') {
                            // CALL links inherit color from Source Procedure Type (Caller)
                            if (src.label.endsWith('%')) strokeColor = "#4CAF50";      // Integer
                            else if (src.label.endsWith('$')) strokeColor = "#E040FB"; // String
                            else strokeColor = "#FF9800";                              // Float/Default
                            ...
                            strokeColor = "#FF9800"; // Default Fallback
                        }
